        ...
        <div className={`w-full h-full overflow-hidden ${dropzoneRadiusClass}`}>
          {isPdf ? (
            ...
            <div className="w-full h-full flex flex-col items-center justify-center bg-gray-50 dark:bg-gray-800 text-gray-400">
              <svg
                className={`${dropzoneSize.icon} mb-1`}
                fill="none"
                stroke="currentColor"
                strokeWidth="1.5"
                viewBox="0 0 24 24"
      ...
      {!disabled && (
        ...
        >
          <img
            src={TrashIcon}
            alt="clear"
            className="w-3.5 h-3.5 transition-all duration-200 group-hover/btn:brightness-0 group-hover/btn:invert"
          />
        </div>
      )}
...
/* -------------------------------------------------------------------------- */
/*                         Main CustomFileInput Component                      */
/* -------------------------------------------------------------------------- */
...
const CustomFileInput = ({
  ...
  const shouldFloat = hasValue || (isFloating && !!placeholder);
    ...
    return (
      <label
        ...
        className={`block font-medium select-none transition-colors duration-200 ${isOutsideLeft ? "shrink-0 mb-0" : "mb-1.5"
          } ${sz.labelSize} ${labelClassName} text-neutral-700 dark:text-neutral-300`}
      >
        {label}
      </label>
    );
        ...
        <div>
          {!previewUrl ? (
            <div {...getRootProps()} className="flex justify-center">
              ...
              >
                <img src={ProfileImage} alt="profile-logo" className="w-full h-full object-cover" />
              </label>
            </div>
          ) : (
                ...
                {!disabled && (
                  ...
                  >
                    <img
                      src={TrashIcon}
                      alt="clear"
                      className="w-4 h-4 transition-all duration-200 group-hover/btn:brightness-0 group-hover/btn:invert"
                    />
                  </div>
                )}
              </div>
          ...
          {!previewUrl || !isSupported ? (
            <div {...getRootProps()} className="relative mt-2" style={{ width: `${dropzoneSizeVal}px`, height: `${dropzoneSizeVal}px` }}>
              <label
                ...
                `}
              >
                <svg className={`${dropzoneSize.icon}`} fill="currentColor" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20">
            ...
            {isFloating && label && (
              <motion.label
                ...
                className={`
                  ...
                  ${sz.textSize} ${labelClassName} ${shouldFloat ? "text-neutral-700 dark:text-neutral-300" : "text-neutral-400 dark:text-neutral-500"
                  }
                `}
              >
                {label}
