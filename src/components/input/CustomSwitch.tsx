...
const CustomSwitch = forwardRef<HTMLDivElement, CustomSwitchProps>((props, ref) => {
  const {
    label,
    activeLabel,
    inactiveLabel,
  ...
  return (
    <div className={`w-full ${containerClassName}`} ref={ref}>
      ...
      {label && (
        <label
          className={`block text-sm font-medium text-neutral-700 dark:text-neutral-300 mb-2 ${labelClassName}`}
        >
          {label}
        </label>
      )}
      ...
      <div className="flex items-center gap-3 mt-1">
        ...
        {(activeLabel || inactiveLabel) && (
          <span
            onClick={handleToggle}
            className={`text-sm select-none font-medium cursor-pointer transition-colors duration-200 ${
              currentValue
